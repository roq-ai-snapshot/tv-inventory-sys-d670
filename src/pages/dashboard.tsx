...
import { getTvs } from 'apiSdk/tvs';
import { TvInterface } from 'interfaces/tv';

import { TvListPage as ShowCaseList } from 'pages/tvs';

function DashboardPage() {
  const { session } = useSession();

  const tvStat = useSWR<PaginatedInterface<TvInterface>>(
    () => `/tvs`,
    () => getTvs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Tvs',
      result: tvStat?.data?.totalCount,
      isLoading: tvStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`tvs`} passHref legacyBehavior>
                  <Button
